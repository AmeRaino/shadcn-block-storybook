...
const MyFloatingLabel = ({
  ...
  return (
    <Label
      className={cn(
        "absolute start-2 top-2 z-10 origin-[0] -translate-y-4 scale-75 transform bg-background px-2 text-sm text-gray-500 duration-300 peer-placeholder-shown:top-1/2 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:scale-100 peer-focus:top-2 peer-focus:-translate-y-5 peer-focus:scale-75 peer-focus:px-2 dark:bg-background rtl:peer-focus:left-auto rtl:peer-focus:translate-x-1/4 cursor-text",
        className
      )}
      {...props}
    />
...
const MyFloatingLabelInput = ({
  ...
  return (
    <MyInput
      ...
      {...restInputProps}
    >
      <MyFloatingLabel htmlFor={id}>{label}</MyFloatingLabel>
    </MyInput>
  );
};
